export class SelfPrompter {
    ...
    setPrompt() {
        const currentIdleTime = Date.now() - this.agent.lastActionTime;
        const externalTimeout = (settings.external_prompt_timeout_seconds || 10) * 1000;
        const isSystemIdle = !this.agent.actionManager.executing;
        const noRecentExternal = Date.now() - this.lastExternalPromptTime > externalTimeout;

        if (currentIdleTime > this.idleThreshold && isSystemIdle && noRecentExternal) {
            this._triggerSelfPrompt();
        }
    }
