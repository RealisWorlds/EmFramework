...
class ItemWrapper {
    ...
    createChildren() {
        ...
        if (recipes.length === 0) {
            console.warn(`No crafting recipes found for ${this.name}`);
        }
        ...
        for (let recipe of recipes) {
            ...
            try {
                let itemNode = new ItemNode(this.manager, this, this.name).setRecipe(recipe);
                this.add_method(itemNode);
            } catch (error) {
                console.error(`Error setting recipe for ${this.name}:`, error);
            }
        }
        ...
        if (block_sources.length > 0 && this.name !== 'torch' && !this.name.includes('bed')) {
            for (let block_source of block_sources) {
                if (block_source === 'grass_block') continue;
                try {
                    ...
                    console.error(`Error setting collectable for ${this.name} (block: ${block_source}):`, error);
                }
            }
        }
        ...
        if (smeltingIngredient) {
            try {
                ...
                console.error(`Error setting smeltable for ${this.name}:`, error);
            }
        }
        ...
        if (animal_source) {
            try {
                ...
                console.error(`Error setting huntable for ${this.name}:`, error);
            }
        }
    }
